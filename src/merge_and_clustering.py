...
def make_X_merged_only(merged: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], List[str]]:
    """
    Create features from MERGED dataset excluding:
    - part_id (identifier)
    - fried (label; excluded from clustering)
    - forbidden frailty-defining variables (per assignment)
    """
    forbidden = [
        "weight_loss",
        "exhaustion_score",
...
def evaluate_grid_kmeans_agnes_gmm(
    ...
    Returns:
    - runs_df (all runs)
    - best_kmeans (dict + labels)
    - best_agnes  (dict + labels)
    - best_gmm    (dict + labels)
    """
    all_runs = []
    best_kmeans = None
    best_agnes = None
